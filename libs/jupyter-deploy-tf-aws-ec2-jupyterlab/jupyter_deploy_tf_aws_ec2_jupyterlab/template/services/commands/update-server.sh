...
#   sudo sh update-server.sh start [all|jupyter|traefik|auth-sidecar]
#   sudo sh update-server.sh stop [all|jupyter|traefik|auth-sidecar]
#   sudo sh update-server.sh restart [all|jupyter|traefik|auth-sidecar]
...
validate_args() {
  ...
  if [ "$SERVICE" != "all" ] && [ "$SERVICE" != "jupyter" ]; then
    ...
    echo "Possible values are: 'all', 'jupyter', 'traefik' or 'auth-sidecar'"
  fi
}
