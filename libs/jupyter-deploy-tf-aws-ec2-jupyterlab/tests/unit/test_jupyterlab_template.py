...
from jupyter_deploy_tf_aws_ec2_jupyterlab.template import TEMPLATE_PATH  # type: ignore
...
def test_aws_dependencies_consistency() -> None:
    ...
    2. The jupyterlab template doesn't specify version requirements for these dependencies

    Related to: https://github.com/jupyter-infra/jupyter-deploy/issues/79
    """
    ...
    template_path = Path(__file__).parent.parent.parent / "pyproject.toml"

    # Ensure both files exist
    assert jd_path.exists(), f"jupyter-deploy pyproject.toml not found at {jd_path}"
    assert template_path.exists(), f"jupyterlab template pyproject.toml not found at {template_path}"
    ...
    with open(template_path, "rb") as f:
        template_pyproject = tomllib.load(f)
    ...
    template_deps = {}
    for dep in template_pyproject.get("project", {}).get("dependencies", []):
        # Handle version specifications (e.g., "boto3>=1.38.23" -> "boto3")
        if any(c in dep for c in [">", "<", "="]):
            package_name = dep.split(">")[0].split("<")[0].split("=")[0].strip()
        else:
            package_name = dep
        template_deps[package_name] = dep

    # Check that all AWS dependencies from jupyter-deploy are in the base template
    for package_name in jd_aws_package_names:
        assert package_name in template_deps, f"AWS dependency {package_name} is missing from the base template"
    ...
    for package_name in jd_aws_package_names:
        dep_str = template_deps[package_name]
