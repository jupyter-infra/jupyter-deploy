...
# Read the AUTHED_USERS file that TF created in /etc/
if [ -f /etc/AUTHED_USERS ]; then
    AUTHED_USERS_CONTENT=$(cat /etc/AUTHED_USERS)
    echo "Read authorized users from /etc/AUTHED_USERS: ${AUTHED_USERS_CONTENT}"
else
    echo "Warning: /etc/AUTHED_USERS file not found"
