...
def events(
    ...
    with handle_cli_errors(console), cmd_utils.project_dir(project_dir):
        ...
        with simple_display_manager.spinner("Fetching pool events..."):
            items = handler.list_events()
        ...
        if not items:
            console.print("No pool events found.")
            return

        console.print(f"[bold]Recent pool events ({len(items)}):[/]")
        console.line()
        for item in items:
            ...
            console.print(f"  [bold cyan]{name}[/]  created={creation_time}")
